...
export function validate(payload) {
  if (!payload || typeof payload.raw !== "string" || !payload.raw.trim()) return "Пустой текст.";
  if (payload.raw.length > 20000) return "Слишком длинный кусок текста.";
  return null;
}
...
export async function fixText(payload) {
  ...
  if (message.stop_reason === "refusal") {
    return { text: payload.raw, warning: "Модель отказалась править этот фрагмент — оставил как есть." };
  }
